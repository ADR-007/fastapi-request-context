...
class TestContextVarsAdapter:
    ...
    def test_exit_context_resets_values(self) -> None:
        """Test that exiting context resets values."""
        adapter = ContextVarsAdapter()

        # First context
        adapter.enter_context({"key": "first"})
        assert adapter.get_value("key") == "first"
        adapter.exit_context()

        # New context
        adapter.enter_context({"key": "second"})
        assert adapter.get_value("key") == "second"
        adapter.exit_context()
