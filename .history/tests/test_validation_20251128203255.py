...
class TestCheckRoutesAndDependenciesAreAsync:
    ...
    def test_nested_dependencies(self) -> None:
        """Test that nested dependencies are checked."""

        def inner_sync_dep() -> int:
            return 42

        async def outer_dep(value: int = Depends(inner_sync_dep)) -> int:
            return value * 2

        app = FastAPI()

        @app.get("/")
        async def route(value: int = Depends(outer_dep)) -> dict[str, int]:
            return {"value": value}

        warnings = check_routes_and_dependencies_are_async(app)
        # Should detect the sync inner dependency
        assert any("inner_sync_dep" in w for w in warnings)
