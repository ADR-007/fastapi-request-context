...
handler = logging.StreamHandler()
handler.setFormatter(LocalContextFormatter(
...
- `FastAPIWrapperMiddleware(app)` - Base class for custom middleware
...
- `ContextLoggingAdapter` - Adapter using context-logging library

### Formatters

- `JsonContextFormatter` - JSON formatter for production
- `LocalContextFormatter` - Human-readable formatter for development
